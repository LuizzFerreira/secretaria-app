...
import { Plus, Search, X, Trash2, Edit3, Check, StickyNote, Clock } from 'lucide-react'
...
function CardAnotacao({ nota, onEditar, onExcluir }) {
  const corConfig = CORES.find(c => c.id === nota.cor) || CORES[0]
  const dataFormatada = new Date(nota.atualizadoEm).toLocaleDateString('pt-BR', {
    ...
    <div className="space-y-6">
      ...
      {filtradas.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtradas.map(nota => (
            <CardAnotacao key={nota.id} nota={nota} onEditar={handleEditar} onExcluir={handleExcluir} />
          ))}
        </div>
      )}
    </div>
